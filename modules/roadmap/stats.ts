...
import { documentNodes, documents, documentResources, nodeResources, reviewRuns, reviewFindings } from "../../db/schema";
export async function nodeStats(db: Database, nodeIds: string[]) {
  ...
  for (const id of nodeIds) {
    ...
    stats.set(id, { documents: docs.size, sources: sources.size, openFindings: findings.filter((f) => f.status === "OPEN" && runs.some((r) => r.id === f.reviewRunId)).length, outdatedReviews: linked.filter((link) => { const run = latest.get(link.documentId); return run && run.revisionId !== link.currentRevisionId; }).length });
  }
  return stats;
}
