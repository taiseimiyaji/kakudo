...
export function createSearchProvider(env: Record<string, string | undefined> = process.env, signal?: AbortSignal): SearchProvider {
  ...
  if (name === "codex") {
    const transport = codexTransport({ model: env.CODEX_MODEL, path: env.CODEX_PATH, searchOnly: true, signal });
  ...
  if (name === "openai") return { async search(query) {
    ...
    const response = await client.responses.create({ model: env.OPENAI_MODEL, instructions: SEARCH_POLICY, input: JSON.stringify({ query }), tools: [{ type: "web_search" }], store: false }, { signal });
