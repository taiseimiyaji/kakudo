...
it("correlates unexpected errors without logging request secrets or raw errors", async () => {
  ...
  try {
    const app = createApp({ checkDatabase: async () => {}, findWorkspace: async () => null, database: () => { throw new Error("postgres://user:secret@host learner text"); } });
    const response = await app.request("/api/roadmaps?token=secret", { method: "POST", headers: { Origin: "http://127.0.0.1:43171", "Content-Type": "application/json", Authorization: "secret", "X-Request-ID": "secret" }, body: '{"title":"learner text"}' });
...
it("reports storage and database failures separately using safe cause codes", async () => {
  const log = vi.spyOn(console, "error").mockImplementation(() => {});
  try {
    const app = createApp({ checkDatabase: async () => {}, findWorkspace: async () => null, checkStorage: async () => { throw Object.assign(new Error("private path"), { code: "EACCES" }); } });
