...
調査後に#22〜#28の実装・検証を順次PR化した。CSRF対策、E2E分離、バックアップ復元、単一プロセス所有、サービス設定生成、Review受付制限・期限、障害ログ・保存領域healthを追加。[運用手順](operations.md)と[Issue別検証記録](verification/issue-28.md)を参照。

このMacの一時launchdサービスで異常終了後の自動再起動・データ維持・中断Review再実行と、実CodexのReview・検索を確認した。恒久配置とホスト再起動は#25で未完了。公開先・許可利用者が未確定のため、#29は[公開設定・受け入れ手順](public-access.md)のみ整備し、実設定とHTTPS経路の検証は未実施。全体の残作業は[追跡Issue #30](https://github.com/taiseimiyaji/kakudo/issues/30)で管理する。

以下は修正前commitに対する調査記録であり、現在の未対応一覧ではない。
