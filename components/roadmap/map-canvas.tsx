...
export function MapCanvas({ detail, selected, onSelect, onMove, onConnect, onDeleteEdge, busy }: {
  ...
  const [nodes, , onNodesChange] = useNodesState<MapNode>(detail.nodes.map((n) => ({ id: n.id, type: "learning", data: { title: n.title, status: n.status, stats: n.stats }, position: { x: n.positionX, y: n.positionY } })));
  return <>
    ...
    <div className="flow-canvas" aria-label="学習マップ">
      <ReactFlow nodes={nodes.map((n) => ({ ...n, data: { ...n.data, stats: detail.nodes.find((node) => node.id === n.id)?.stats ?? n.data.stats }, selected: n.id === selected }))} edges={detail.edges.map((e) => ({ id: e.id, source: e.sourceId, target: e.targetId, label: e.type, markerEnd: e.type === "RELATED" ? undefined : { type: MarkerType.ArrowClosed }, style: e.type === "RELATED" ? { strokeDasharray: "5 5" } : {} }))}
